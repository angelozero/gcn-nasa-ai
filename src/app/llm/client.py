...
class LLMClient:
    ...
    def __init__(self, settings: GCNNasaSettings) -> None:
    ...
    def chat(self, model: str, messages: list[dict]) -> str:
        ...
        Args:
            model: Alias do modelo no LiteLLM (ex: ``"nasa-classifier"``).
        ...
        logger.debug("chat() tokens_resposta=%d", response.usage.total_tokens if response.usage else 0)
        return content
    ...
    def embed(self, text: str) -> list[float]:
        ...
        """
        logger.debug("embed() model=%s tamanho_texto=%d", self._settings.LLM_MODEL_EMBEDDER, len(text))
        response = self._client.embeddings.create(
            model=self._settings.LLM_MODEL_EMBEDDER,
            input=text,
        )
